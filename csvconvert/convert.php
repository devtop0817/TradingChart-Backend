        ...
        if (strtotime($csvdate) >= strtotime($lastupdateddate)) {
            ...
            if (($handle = fopen($dirval . '/' . $filevalues, "r")) !== false) {
                ...
                while (($data = fgetcsv($handle)) !== false) {
                    ...
                    if (($data[0] == 'SYMBOL') || ($data[1] == 'DATE') || ($data[0] == 'symbol') || ($data[1] == 'date')) {
                        ...
                        //if( (strtotime($cdate) <= strtotime($lastupdateddate)) || ($lastupdateddate == '0000-00-00') ){
                        //if( $checkflag == 0 ){
                        $checkdelsql = "Delete from " . $tablename . " WHERE csymbol = '" . $csymbol . "' AND cdate = '" . $cdate . "'  ";
                        //$checkdelsql = "Delete from ".$tablename." WHERE cdate = '".$cdate."'  ";
                        if (sqlsrv_query($conn1, $checkdelsql) === false) {

                        } else {
                            $checkflag = 1;
                            echo "Removing duplicated data..." . $market . "\n";
                        }
